...
// API Configuration
const API_BASE_URL = 'https://workspace-codenurse.replit.app/api';
const LOCAL_API_URL = 'http://localhost:5000/api/test';
...
const apiRequest = async (endpoint, options = {}) => {
  try {
    ...
    console.log('Public API failed, trying local:', error.message);
    ...
    try {
      ...
      console.log('Local API also failed:', localError.message);
      throw new Error('Both APIs unreachable');
    }
  }
};
...
export default function App() {
  ...
  // Load trips from API and storage
  const loadTrips = async () => {
    try {
      ...
      if (!currentDeviceId) {
        try {
          const storedDeviceId = await Keychain.getInternetCredentials('miletracker_device_id');
          if (storedDeviceId && storedDeviceId.password) {
            currentDeviceId = storedDeviceId.password;
          } else {
            currentDeviceId = generateDeviceId();
            await Keychain.setInternetCredentials('miletracker_device_id', 'device', currentDeviceId);
          }
          setDeviceId(currentDeviceId);
        } catch (error) {
          console.log('Device ID error, using temporary:', error.message);
          currentDeviceId = generateDeviceId();
          setDeviceId(currentDeviceId);
        }
      }

      // Try to load trips from API
      try {
        const response = await apiRequest(`/device/trips?deviceId=${currentDeviceId}`);
        if (response.trips && Array.isArray(response.trips)) {
          setTrips(response.trips);
          calculateMonthlyStats(response.trips);
          console.log(`Loaded ${response.trips.length} trips from API`);
          return;
        }
      } catch (apiError) {
        console.log('API load failed, trying local storage:', apiError.message);
      }
      ...
      if (credentials && credentials.password) {
        ...
        console.log(`Loaded ${parsedTrips.length} trips from local storage`);
      } else {
  ...
  // Save a new trip to API and local storage
    ...
    try {
      ...
      if (deviceId) {
        try {
          const tripData = {
            deviceId: deviceId,
            start_location: newTrip.startAddress || newTrip.startLocation || 'Unknown',
            end_location: newTrip.endAddress || newTrip.endLocation || 'Unknown', 
            distance: parseFloat(newTrip.distance) || 0,
            category: newTrip.category || 'Business',
            purpose: newTrip.purpose || newTrip.method || 'Auto-detected'
          };
          
          const response = await apiRequest('/device/trips', {
            method: 'POST',
            body: JSON.stringify(tripData)
          });
          
          if (response.trip || response.id) {
            console.log('Trip saved to API successfully');
            savedTrip = { ...newTrip, apiId: response.trip?.id || response.id };
          }
        } catch (apiError) {
          console.log('API save failed, using local only:', apiError.message);
        }
      }

      // Update local state and save to local storage as backup
      const updatedTrips = [...trips, savedTrip];
